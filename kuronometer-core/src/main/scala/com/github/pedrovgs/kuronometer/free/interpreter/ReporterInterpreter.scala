...
import cats.{Id, ~>}
import com.github.pedrovgs.kuronometer.free.algebra.{GetTodayBuildExecution, GetTotalBuildExecution, ReportBuildExecution, ReporterOp}
...
class ReporterInterpreter(implicit csvReporter: CsvReporter, apiClient: KuronometerApiClient, clock: Clock)
  ...
  override def apply[A](fa: ReporterOp[A]): Id[A] = fa match {
    case ReportBuildExecution(buildExecution, RemoteReport) => apiClient.report(buildExecution)
    case ReportBuildExecution(buildExecution, LocalReport) => csvReporter.report(buildExecution)
    case GetTotalBuildExecution() => csvReporter.getTotalBuildExecutionStages
    case GetTodayBuildExecution() => csvReporter.getBuildExecutionStagesSinceTimestamp(clock.todayMidnightTimestamp)
  }
