...
object SummaryBuildStageExecutionFormatter {
  ...
  def format(execution: SummaryBuildStagesExecution): String = {
    ...
    val stages = execution.buildStages
      ...
      .map {
        case (name: String, executionTime: Long) =>
          SummaryBuildStageExecution(name, executionTime, 0)
      }
      .toList
      .sortWith { (lhs, rhs) =>
        lhs.executionTimeInNanoseconds > rhs.executionTimeInNanoseconds
      }
    val chart = if (stages.nonEmpty) {
      stages
        .map { stage =>
          format(stage, totalExecutionTime)
        }
        .mkString("", "\n", "\n")
    } else {
      ""
  ...
  private def format(buildStage: SummaryBuildStageExecution,
                     totalExecutionTime: Long): String = {
    val executionTime =
      NanosecondsFormat.format(buildStage.executionTimeInNanoseconds)
