...
object BuildExecutionGenerators {
  ...
  implicit val arbBuildExecution: Arbitrary[BuildExecution] = Arbitrary(
    buildExecution())
  implicit val arbBuildStagesExecution: Arbitrary[BuildStagesExecution] =
    Arbitrary(buildStagesExecution())
  implicit val arbSummaryBuildExecution
    : Arbitrary[SummaryBuildStagesExecution] = Arbitrary(
  ...
  def summaryBuildStagesExecution: Gen[SummaryBuildStagesExecution] =
    for {
      stages <- Gen.listOf(summaryBuildStageExecution)
    } yield SummaryBuildStagesExecution(stages)

  def summaryBuildStageExecution: Gen[SummaryBuildStageExecution] =
    for {
      name <- buildStageName
  ...
  def buildExecution(timestamp: Gen[Long] = timestamp): Gen[BuildExecution] =
    for {
  ...
  def buildStageExecution(
      timestamp: Gen[Long] = timestamp): Gen[BuildStageExecution] =
    for {
      executionTime <- buildExecutionTime
      timestamp <- timestamp
