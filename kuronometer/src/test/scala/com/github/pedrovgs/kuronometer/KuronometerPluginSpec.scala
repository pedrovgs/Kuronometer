...
class KuronometerPluginSpec extends FlatSpec with Matchers {
  ...
  "KuronometerPlugin" should "register two tasks to retrieve build execution times" in {
    project.getTasks.findByName(GetTodayBuildTimeSummaryTask.name) shouldBe a[
      GetTodayBuildTimeSummaryTask]
    project.getTasks.findByName(GetTotalBuildTimeSummaryTask.name) shouldBe a[
      GetTotalBuildTimeSummaryTask]
  }

  it should "register a kuronometer extension" in {
    project.getExtensions.findByName(KuronometerExtension.name) shouldBe a[
      KuronometerExtension]
  }
}
